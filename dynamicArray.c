...
void removeMinHeap(DynArr *heap)
{
	...
	int lastIdx = sizeDynArr(heap) - 1;

	putDynArr(heap, 0, getDynArr(heap, lastIdx));
	removeAtDynArr(heap, lastIdx);

	_adjustHeap(heap, lastIdx, 0);
}
